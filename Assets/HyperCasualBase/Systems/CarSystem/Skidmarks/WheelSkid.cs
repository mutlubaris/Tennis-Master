...
public class WheelSkid : MonoBehaviour
{
    ...
    public Rigidbody rb { get { return (rbody == null) ? rbody = GetComponentInParent<Rigidbody>() : rbody; } }
    ...
    protected void LateUpdate()
    {
        if (wheelCollider.GetGroundHit(out wheelHitInfo))
        {
            ...
            if (skidTotal >= SKID_FX_SPEED)
            {
                ...
                lastSkid = SkidmarksController.Instance.AddSkidMark(skidPoint, wheelHitInfo.normal, intensity, lastSkid);
            }
            else
                lastSkid = -1;
        }
